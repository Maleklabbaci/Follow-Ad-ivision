  ...
  return (
    <div className="flex h-screen overflow-hidden bg-slate-50">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        {isImpersonating && (
          <div className="bg-amber-500 text-white px-4 md:px-6 py-2 flex flex-col sm:flex-row items-center justify-between gap-2 shadow-md z-10 text-center sm:text-left">
            <div className="flex items-center gap-2 text-[10px] md:text-sm font-bold uppercase tracking-tight">
              <svg className="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
              </svg>
              Simulation: {impersonatedClient?.name || 'Client'}
            </div>
            <button 
              onClick={() => navigate('/admin/clients')}
              className="bg-white/20 hover:bg-white/30 px-3 py-1 rounded text-[10px] font-black transition-colors border border-white/40 uppercase"
            >
              Quitter l'Audit
            </button>
          </div>
        ...
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 md:px-6 shrink-0 z-20">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-sm md:text-lg font-black text-slate-900 tracking-tighter uppercase truncate max-w-[150px] md:max-w-none">
