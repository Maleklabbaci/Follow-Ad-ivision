...
interface AdiVisionChatbotProps {
  secrets: IntegrationSecret[];
  campaigns?: CampaignStats[];
  activeClientName?: string;
}

const AdiVisionChatbot: React.FC<AdiVisionChatbotProps> = ({ secrets, campaigns = [], activeClientName }) => {
  const [isOpen, setIsOpen] = useState(false);
  const [messages, setMessages] = useState<Message[]>([
    { role: 'bot', content: "Bienvenue sur AdiVision AI ! 🚀 Je suis votre guide. Voulez-vous savoir pourquoi nous sommes plus efficaces qu'une agence traditionnelle ?" }
  ]);
  ...
  const handleSend = async (customMsg?: string) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'bot', content: "L'IA d'AdiVision est en cours de recalibrage. Posez-moi une autre question !" }]);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const suggestedQuestions = activeClientName 
    ...
    : ["Pourquoi AdiVision ?", "Est-ce sécurisé ?", "Parler à un expert"];
        ...
        <div className="mb-4 w-[350px] md:w-[400px] h-[550px] bg-white rounded-[2.5rem] border border-slate-200 shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-bottom-10 duration-500">
              ...
              <div>
                ...
                <div className="flex items-center gap-1">
                   <span className="text-[9px] font-bold text-emerald-400 uppercase tracking-widest">
                     {activeClientName ? `Analyste: ${activeClientName}` : 'AdiVision Active'}
                   </span>
          ...
          <div className="p-4 bg-white border-t border-slate-100 space-y-3">
            ...
            <div className="flex gap-2">
              <input 
                type="text" 
                placeholder="Posez votre question à AdiVision..."
